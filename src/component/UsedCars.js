...
const fetchCarsData = async (budget, selectedFilters) => {
  try {
    ...
    const response = await fetch(
      `http://13.233.193.243:3000/api/cars/cars?budgetMin=${budget[0]}&budgetMax=${budget[1]}&filters=${filterQuery}`,
      {
        method: 'GET',
        headers: {
...
function UsedCars() {
  ...
  const handleAddToCart = async (carId) => {
    try {
      const response = await fetch('http://13.233.193.243:3000/api/cart/add', {
        method: 'POST',
        credentials: 'include',
        headers: {
  ...
  const handleRemoveFromCart = async (carId) => {
    try {
      const response = await fetch(`http://13.233.193.243:3000/api/cart/remove/${carId}`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
