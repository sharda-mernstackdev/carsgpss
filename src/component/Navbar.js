...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      setIsLoading(true);
      try {
        const response = await fetch('http://43.204.217.35:3000/api/users/user-detail', {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const fetchCartData = async () => {
      try {
        const response = await fetch('http://43.204.217.35:3000/api/cart', {
          method: 'GET',
          credentials: 'include',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  const handleLogout = async () => {
    ...
    try {
      const response = await fetch('http://43.204.217.35:3000/api/users/userLogout', {
        method: 'GET',
        credentials: 'include',
      });
