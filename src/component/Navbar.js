...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      setIsLoading(true);
      try {
        const response = await fetch('http://13.201.85.67:3001/api/users/user-detail', {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const fetchCartData = async () => {
      try {
        const response = await fetch('http://13.201.85.67:3001/api/cart', {
          method: 'GET',
          credentials: 'include',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  const handleLogout = async () => {
    ...
    try {
      const response = await fetch('http://13.201.85.67:3001/api/users/userLogout', {
        method: 'GET',
        credentials: 'include',
      });
