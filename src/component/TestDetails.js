...
const TestDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`http://43.204.217.35:3001/api/testDrive/get/${id}`, {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    const fetchCarData = async (carId) => {
      ...
      const url = `http://43.204.217.35:3001/api/cars/cars/${carIdString}`;
  ...
  const handleCancel = async () => {
    try {
      const response = await fetch(`http://43.204.217.35:3001/api/testDrive/${id}/cancel`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
  ...
  const confirmDelete =async () => {
    try {
      const response = await fetch(`http://43.204.217.35:3001/api/testDrive/delete/${id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
